...
yaml_QDA_path = [datapath 'qda_cvsa/cfg/qda_c7_26112025_102244.yaml'];
...
for idx_file= 1: nFiles
    ...
    headers = c_header;
    raw_signals = cat(1, raw_signals, c_signal);
end
...
for idx_feature = 1:length(features)
    ...
    prob_qda_matlab = cat(1, prob_qda_matlab, c_prob);
end

%% ----------------- REIMM -----------------
% save data of the evaluation
save('./ic_cvsa_ws/src/analysis_cvsa/riemaniann/data_evaluation.mat', 'raw_signals')

% load data of the predict
d_r = load('./ic_cvsa_ws/src/analysis_cvsa/riemaniann/raw_prob_riemm.mat');
tmp_riem = d_r.preds;
prob_riem = zeros(size(gmm_prob_matlab, 1), 1);
prob_riem(end-size(tmp_riem, 2)+1:end) = tmp_riem;

%% ----------------- ACC -------------------
mask_trial = zeros(size(gmm_prob_matlab, 1), 1);
true_label = zeros(size(gmm_prob_matlab, 1), 1);
for i = 1:ntrial
    mask_trial(cfPOS(i):cfPOS(i) + cfDUR(i) - 1) = 1;
    true_label(cfPOS(i):cfPOS(i) + cfDUR(i) - 1) = cueTYP(i);
end
mask_gmm_AND_trial = (mask_trial == 1) & (gmm_prob_matlab(:,ic_index) >= threshold_gmm_ic) & (artifacts == 0);
true_label(~mask_gmm_AND_trial) = 0;
qda_hard = prob_qda_matlab(:,1);
qda_hard(find(qda_hard >= 0.5)) = qda.classes(1);
qda_hard(find(qda_hard < 0.5)) = qda.classes(2);
qda_acc = sum(qda_hard == true_label)/sum(mask_gmm_AND_trial)*100

riem_acc = sum(prob_riem == true_label)/sum(mask_gmm_AND_trial)*100


%% ----------------- plot prob integrated -----------------
alpha = 0.98;
bufferSize = 48;
for idx_trial =50:ntrial
    ...
    c_riem = prob_riem(start_trial:end_trial, 1) - 730;
    ...
    exponential = ones(nsamples_trial, 1)*1/nclasses;
    ...
    for idx_sample = 1:nsamples_trial
        if idx_sample >= trial_start_cf
            if c_gmm_prob(idx_sample,ic_index) >= threshold_gmm_ic && c_artifact(idx_sample) == 0
                ...
                mask = cat(1, mask, 1);
                exponential(idx_sample) = exponential(idx_sample-1) * alpha + (1-alpha) * tmp;
            else
                mask = cat(1, mask, 0);
                exponential(idx_sample) = exponential(idx_sample-1);
    ...
    hold off
    ylim([0 1])
    legend('qda prob','riem','qda not used', 'qda prob used')
