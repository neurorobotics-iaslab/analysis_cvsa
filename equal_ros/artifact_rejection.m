...
disp('      [INFO] start artifact rejection: EOG an EMG' );
...
for i=1:nchunks
    ...
    % --- muscle artefact part ---
    [data_art,zi_high] = filter(b_high,a_high,buffer,zi_high);

    data_non_eog = data_art(:,non_eog);
    ...
    [data_eog,z_low_eog] = filter(b_low_eog,a_low_eog,buffer,z_low_eog);
    [data_eog,z_high_eog] = filter(b_high_eog,a_high_eog,data_eog,z_high_eog);

    heog = data_eog(:, eog_idx(1)) - data_eog(:, eog_idx(2));
    if size(eog_idx, 2) == 2
        % we have only FP1 and FP2
        veog = (data_eog(:, eog_idx(1)) + data_eog(:, eog_idx(2))) / 2;
    else
        veog = ((data_eog(:, eog_idx(1)) + data_eog(:, eog_idx(2))) / 2) - data_eog(:, eog_idx(3));
    end
