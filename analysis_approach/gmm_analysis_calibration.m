...
for idx_file = 1:nFiles
    ...
    peaks(idx_file) = analyze_alpha_peak(fullpath_file, 'RestTrigger', 786);
end
...
if exist('topoplot', 'file')
    ...
    title({'All Data (No Filter)', ['Mean Diff (' num2str(classes(1)) ' - ' num2str(classes(2)) ')']}, 'FontSize', 12, 'FontWeight', 'bold');
    colorbar;
    
    % Colormap: Rosso = Classe 1 Maggiore, Blu = Classe 2 Maggiore
    colormap(jet); 
    sgtitle(['Differenza di Potenza Media (' num2str(bands{choosen_band}(1)) '-' num2str(bands{choosen_band}(2)) ' Hz)'], 'FontSize', 14);
