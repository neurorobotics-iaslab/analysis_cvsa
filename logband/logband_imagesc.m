...
%% baseline correction log band with correction
...
for idx_band = 1:nbands
    ...
    handle = [handle, gca];
    c_max = max(c_max, max(abs(avg_correction_1), [], 'all'));
    ...
    handle = [handle, gca];
    c_max = max(c_max, max(abs(avg_correction_2), [], 'all'));
    idx_plot = idx_plot + 1;
...
if baseline_correction
    sgtitle('Log band in time with baseline correction')
else
    sgtitle('Log band in time without baseline correction')
end
