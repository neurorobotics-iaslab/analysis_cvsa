...
path = ['/home/paolo/cvsa_ws/record/' subject '/20240926/gdf/' a];
...
for idx_f = 1:length(files)
    ...
    for idx_band=1:size(bands, 1)
        ...
        if ~isempty(chanlog_path)
            showTopoplot_ERDERS(chanlog_path, ERD, sampleRate, 2, channels_label, band, minDurTrial, cueTYP, classes, nchannels, ['matlab' files(idx_f).name])
        end
    end
end
...
for idx_band=1:size(bands,1)
    ...
    % Visualization
    if ~isempty(chanlog_path)
        showTopoplot_ERDERS(chanlog_path, total.ERD{idx_band}, sampleRate, 2, channels_label, band, total.minDurTrial(idx_band), total.cueTYP, classes, nchannels, 'all files concatenated')
    end
end
