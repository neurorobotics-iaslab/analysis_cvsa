...
for idx_file= 1: nFiles
    ...
    trial_with_eog = [trial_with_eog; c_trial_with_eog];
     ...
     for idx_band = 1:nbands
        ...
        signal_processed = 10*log10(signal_processed + eps);
        ...
        if isempty(find(header_processed.EVENT.TYP == 2, 1)) % no eye calibration
            ...
            c_header.POS = cat(1, c_header.POS, header_processed.EVENT.POS(k:end) + size(signals{idx_band}, 1));
        end
...
for idx_band = 1:nbands
    for idx_ch=1:nchannels
        c_data = trial_data(min_durFIX + min_durCUE+1:end,idx_band, idx_ch, trial_typ == classes(1));
        mu(1, idx_ch, idx_band) = mean(squeeze(c_data), 'all');
        sigma(1, idx_ch, idx_band) = std(squeeze(c_data(:)));

        c_data = trial_data(min_durFIX + min_durCUE+1:end,idx_band, idx_ch, trial_typ == classes(2));
...
%% Computing AUC per channel
trial_data_mean = nan(nbands, nchannels, ntrial);
for b = 1:nbands
    for t = 1:ntrial
        for ch = 1:nchannels
            trial_data_mean(b,ch,t) = mean(squeeze(trial_data(:,b,ch,t)));
        end
    end
end

labels = trial_typ - min(classes);
AUC = zeros(nbands, nchannels);
for b = 1:nbands
    for ch = 1:nchannels
        scores = squeeze(trial_data_mean(b, ch,:));
        [~,~,~,AUC(b,ch)] = perfcurve(labels, scores, 1);
    end
end
...
for idx_band = 1:nbands
    c_data_1 = mean(mean(squeeze(trial_data(min_durFIX + min_durCUE + 1:end,idx_band,:,trial_typ == classes(1))), 3), 1); % mean on trials and then on samples
    c_data_2 = mean(mean(squeeze(trial_data(min_durFIX + min_durCUE + 1:end,idx_band,:,trial_typ == classes(2))), 3), 1);
...
figure();
idx_plot = 1; handles_all_t = []; cl_all_t = -inf;
handles_cuecf = []; cl_cuecf = -inf;
handles_cf = []; cl_cf = -inf;
for idx_band = 1:nbands
    % all trial
    ...
    idx_plot = idx_plot  + 1;
    handles_all_t = [handles_all_t; gca];
    cl_all_t = max(cl_all_t, max(abs(c_diff), [], 'all'));
    title(['band: ' bands_str{idx_band} ' | all trial']);

    % cue and cf
    data_1 = squeeze(mean(trial_data(min_durFIX + 1:end,idx_band,:,trial_typ == classes(1)), 4));
    data_2 = squeeze(mean(trial_data(min_durFIX + 1:end,idx_band,:,trial_typ == classes(2)), 4));
    c_diff = (data_1 - data_2);

    subplot(3, 3, idx_plot)
    imagesc(c_diff')
    hold on
    xline(min_durCUE, '--r', 'Cf', 'LabelOrientation', 'horizontal');
    hold off
    yticks(1:nchannels)
    yticklabels(channels_label(1:end-1))
    xticks(sampleRate:sampleRate:size(c_diff, 1))
    xticklabels(string(((sampleRate:sampleRate:size(c_diff, 1)) / sampleRate) + ceil(min_durFIX/sampleRate)));
    idx_plot = idx_plot  + 1;
    handles_cuecf = [handles_cuecf; gca];
    cl_cuecf = max(cl_cuecf, max(abs(c_diff), [], 'all'));
    title(['band: ' bands_str{idx_band} ' | only cf and cue']);

    % cf only
    data_1 = squeeze(mean(trial_data(min_durFIX + min_durCUE+1:end,idx_band,:,trial_typ == classes(1)), 4));
    data_2 = squeeze(mean(trial_data(min_durFIX + min_durCUE+1:end,idx_band,:,trial_typ == classes(2)), 4));
    c_diff = (data_1 - data_2);
    ...
    yticklabels(channels_label(1:end-1))
    xticks(sampleRate:sampleRate:size(c_diff, 1))
    xticklabels(string(((sampleRate:sampleRate:size(c_diff, 1)) / sampleRate) + ceil((min_durCUE+min_durFIX)/sampleRate)));
    idx_plot = idx_plot  + 1;
    handles_cf = [handles_cf; gca];
    cl_cf = max(cl_cf, max(abs(c_diff), [], 'all'));
    title(['band: ' bands_str{idx_band} ' | only cf']);
end
set(handles_all_t, 'clim', [-cl_all_t, cl_all_t])
set(handles_cuecf, 'clim', [-cl_cuecf, cl_cuecf])
set(handles_cf, 'clim', [-cl_cf, cl_cf])
sgtitle(['Diff (bl - br) in time | subject: ' subject])
