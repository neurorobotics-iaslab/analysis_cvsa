...
for idx_file = 1:nFiles
    ...
    for idx_bands = 1:nbands
        ...
        [signal_processed, header_processed] = processing_onlineROS_hilbert(c_signal, header, nchannels, bufferSize, filterOrder, c_band, chunkSize, do_hann);
    ...
    if strcmp(paradigm, 'hybrid') || strcmp(paradigm, 'cvsa_blbr')
        ...
        baseline_traj = zeros(size(X));
        c_base = zeros(1, size(X, 2));
        last_update = 1;
        
        for idx_trial = 1:ntrial
            b_start = fixPOS(idx_trial);
            b_end = b_start + fixDUR(idx_trial) - 1;
            
            b_start = max(1, min(b_start, size(X, 1)));
            b_end = max(1, min(b_end, size(X, 1)));
            
            baseline_traj(last_update:b_end, :) = repmat(c_base, b_end - last_update + 1, 1);
            
            c_base = log(mean(X(b_start:b_end, :), 1));
            last_update = b_end + 1;
        end
        if last_update <= size(X, 1)
            baseline_traj(last_update:end, :) = repmat(c_base, size(X, 1) - last_update + 1, 1);
        end
        
        qda_prob_cvsa = apply_qda_matrix(qda_cvsa.model, log(X) - baseline_traj);
    end
    ...
    if strcmp(paradigm, 'hybrid') || strcmp(paradigm, 'mi_lhrh')
        ...
        qda_prob_mi = apply_qda_matrix(qda_mi.model, log(X) - baseline_traj);
    end
    ...
    for idx_trial = 1:ntrial
        ...
        b_start_power = fixPOS(idx_trial);
        b_end_power = fixPOS(idx_trial) + fixDUR(idx_trial) - 1;
        b_start_power = max(1, min(b_start_power, size(signal_processed, 1)));
        b_end_power = max(1, min(b_end_power, size(signal_processed, 1)));
        trial_baseline = log(mean(signal_processed(b_start_power:b_end_power, :), 1));
        
        c_power = log(signal_processed(start_trial:end_trial,:)) - trial_baseline;
