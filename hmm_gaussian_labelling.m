%% HMM to label the samples in time --> following Barzo processing
...
%% Concatenation for train and test
X_train = nan(min_trial_data*max_idx_trial_train, total_electrodes);
T_train = nan(ntrial, 1);
for t = 1: max_idx_trial_train
    T_train(t) = min_trial_data;
    X_train((t-1)*min_trial_data+1:t*min_trial_data,:) = trial_data_train(:,:,t);
end

X_test = nan(min_trial_data*size(trial_data_test,3), total_electrodes);
T_test = nan(ntrial, 1);
for t = 1:size(trial_data_test,3)
    T_train(t) = min_trial_data;
    X_test((t-1)*min_trial_data+1:t*min_trial_data,:) = trial_data_test(:,:,t);
end

%% save the data to try it in python
save('/home/paolo/cvsa_ws/src/analysis_cvsa/hmm/gaussian_hmm.mat',"X_train", 'T_train', 'X_test', 'T_test')
