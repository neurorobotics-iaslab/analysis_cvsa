%% one class classifier using plv values
% clear all; close all;
...
%         s_filt = s_filt * lap;
        ...
        signals{idx_band} = cat(1, signals{idx_band}, signal_processed(:,:)); % for all channels
...
for idx_band = 1:nbands
    ...
    imagesc(squeeze(itcp_1(:,idx_band,channelsSelected))')
    hold on
    ...
    xticks(sampleRate:sampleRate:size(itcp_1, 1))
    xticklabels(string((sampleRate:sampleRate:size(itcp_1, 1)) / sampleRate));
    handle = [handle, gca];
    c_max = max(c_max, max(itcp_1, [], 'all'));
    ...
    subplot(nbands,nclasses + 2,idx_plot)
    imagesc(squeeze(itcp_1(:,idx_band, channelsSelected))' - squeeze(itcp_2(:,idx_band, channelsSelected))')
    hold on
    xline(min_durFIX, '--r', 'Cue', 'LabelOrientation', 'horizontal');
    xline(min_durFIX + min_durCUE, '--r', 'Cf', 'LabelOrientation', 'horizontal');
    hold off
    ...
    xticklabels(string((sampleRate:sampleRate:size(itcp_2, 1)) / sampleRate));
    handle = [handle, gca];
    c_max = max(c_max, max(abs(squeeze(itcp_1(:,idx_band, channelsSelected))' - squeeze(itcp_2(:,idx_band, channelsSelected))'), [], 'all'));
    ...
    subplot(nbands,nclasses + 2,idx_plot)
    imagesc(squeeze(itcp(:,idx_band, channelsSelected))')
    hold on
    xline(min_durFIX, '--r', 'Cue', 'LabelOrientation', 'horizontal');
    xline(min_durFIX + min_durCUE, '--r', 'Cf', 'LabelOrientation', 'horizontal');
    hold off
    ...
    xticklabels(string((sampleRate:sampleRate:size(itcp_2, 1)) / sampleRate));
    handle = [handle, gca];
    c_max = max(c_max, max(abs(squeeze(itcp(:,idx_band, channelsSelected))'), [], 'all'));
...
nrows = 3; ncols = ceil(num_windows/nrows);
for idx_band = 1:nbands
    figure();
    idx_plot = 1; handles = []; cl = -inf;
    for win = 1:num_windows
        start_idx = round((win - 1) * ssize + 1);
        end_idx = round(start_idx + wsize - 1);
        c_itpc = squeeze(itcp(start_idx:end_idx, idx_band, :));

        subplot(nrows, ncols, idx_plot)
        topoplot(squeeze(mean(c_itpc)), chanlocs, 'headrad', 'rim', 'maplimits', [-max(abs(mean(c_itpc))) max(abs(mean(c_itpc)))]);
        cl = max(cl, max(abs(mean(c_itpc))));
        handles = [handles gca];
        axis image;
        colorbar;
        title(['time: ' num2str((start_idx-1)/sampleRate) '-' num2str(end_idx/sampleRate)])
        idx_plot = idx_plot + 1;
    end
    set(handles, 'clim', [-cl, cl]);
    sgtitle(['band: ' bands_str{idx_band}])
end
...
for b = 1:nbands
    ...
    for ch = 1:nchannelsSelected - 1
        ...
        for t = 1:ntrial
            ch_signal(:,t) = circ_var_windows(:, b, channelsSelected(ch), t);
        end
        imagesc(ch_signal');
