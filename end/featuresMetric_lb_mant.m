...
for idx_file= 1: nFiles
    ...
    c_trial_with_eog = eog_detection(c_signal, header, 40, {'FP1', 'FP2', 'EOG'});
...
for idx_ch=1:nchannelsSelected
    ...
    for idx_band=1:nbands
        mean_c1(idx_ch, idx_band) = mean(trial_data_train(c_start:end,idx_band,idx_chSel,trial_typ_train == classes(1)), 'all');
        mean_c2(idx_ch, idx_band) = mean(trial_data_train(c_start:end,idx_band,idx_chSel,trial_typ_train == classes(2)), 'all');
        std_c1(idx_ch, idx_band) = std(trial_data_train(c_start:end,idx_band,idx_chSel,trial_typ_train == classes(1)), 0, 'all');
        std_c2(idx_ch, idx_band) = std(trial_data_train(c_start:end,idx_band,idx_chSel,trial_typ_train == classes(2)), 0, 'all');
    end
end
figure();
...
if create_dataset
    min_durCF = min(cfDUR);
    start_period = time; % s ---> time to start the creation of dataset (from the end of cue)
    period = [ceil(start_period*sampleRate), min_trial_data - minDurCue]; % period of interest in samples
    samples = period(2) - period(1);
    info.files = filenames;
    info.channelsLabel = channels_label;
    info.startTrial    = nan(ntrial,1);
    c_start = minDurCue + period(1) + 1;
        ...
        for idx_trial = 1:ntrial
            ...
            for idx_f = 1:c_nf
                ...
                X((idx_trial-1)*samples+1:idx_trial*samples, idx_f) = ...
                    trial_data(c_start:c_end,c_band, c_channel, idx_trial);
