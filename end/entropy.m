...
entropy_matrix_730 = zeros(num_windows, nchannels,  ntrial/nclasses);
entropy_matrix_731 = zeros(num_windows, nchannels,  ntrial/nclasses);
i_730 = 1; i_731 = 1;

for t = 1:ntrial
    c_signal = trial_data(:,1,:,t);

    trial_entropy = eeg_entropy_windowed(c_signal, 'shannon', window_size, step_size);
    if trial_typ(t) == 730
        entropy_matrix_730(:,:,i_730) = trial_entropy;
        i_730 = i_730 + 1;
    else
        entropy_matrix_731(:,:,i_731) = trial_entropy;
        i_731 = i_731 + 1;
    end
...
figure();
imagesc(mean(entropy_matrix_730, 3)')
xlabel('windows')
ylabel('channels')
yticks(1:nchannels);
yticklabels(header.Label)

xline(floor((min(fixDUR) - window_size)/step_size), 'r--', 'LineWidth', 2);
xline(floor((min(fixDUR) + minDurCue - window_size)/step_size), 'r--', 'LineWidth', 2);
    ...
    for ch = 1:num_channels
        ...
        switch lower(method)
            case 'shannon'
                ...
                nbins = 50; % dividend the range 0-1 into 50 bins
