...
%% GMM -> cnbsmr_train style
disp('------------------- GMM -------------------')
nprototypes = 5;
disp('--- IC ---')
% train
IC_train_data_c1 = IC_train_data(IC_train_labels == classes(1),:);
IC_train_data_c2 = IC_train_data(IC_train_labels == classes(2),:);
IC_GMM_c1 = fitgmdist(IC_train_data_c1(:,IC_idx), nprototypes, 'CovarianceType','diagonal', 'Replicates',10, 'RegularizationValue', 1e-6, 'SharedCovariance', false);
IC_GMM_c2 = fitgmdist(IC_train_data_c2(:,IC_idx), nprototypes, 'CovarianceType','diagonal', 'Replicates',10, 'RegularizationValue', 1e-6, 'SharedCovariance', false);
% acc train
[~, ~, p_IC_train] = predictGMM(IC_GMM_c1, IC_GMM_c2, IC_train_data(:,IC_idx), classes);
confusionmat(IC_train_labels, p_IC_train)
disp(['Accuracy TRAIN: ' num2str(mean(p_IC_train == IC_train_labels))])
% acc test
[~, ~, p] = predictGMM(IC_GMM_c1, IC_GMM_c2, IC_test_data(:,IC_idx), classes);
confusionmat(IC_test_labels, p)
disp(['Accuracy TEST: ' num2str(mean(p == IC_test_labels))])

disp('--- classical ---')
cl_train_data_c1 = cl_train_data(cl_train_labels == classes(1),:);
cl_train_data_c2 = cl_train_data(cl_train_labels == classes(2),:);
cl_GMM_c1 = fitgmdist(cl_train_data_c1(:,cl_idx), nprototypes, 'CovarianceType','diagonal', 'Replicates',10, 'RegularizationValue', 1e-6, 'SharedCovariance', false);
cl_GMM_c2 = fitgmdist(cl_train_data_c2(:,cl_idx), nprototypes, 'CovarianceType','diagonal', 'Replicates',10, 'RegularizationValue', 1e-6, 'SharedCovariance', false);
[~, ~, p_IC_train] = predictGMM(cl_GMM_c1, cl_GMM_c2, cl_train_data(:,cl_idx), classes);
confusionmat(cl_train_labels, p_cl_train)
disp(['Accuracy TRAIN: ' num2str(mean(p_cl_train == cl_train_labels))])
% acc test
[~, ~, p] = predictGMM(cl_GMM_c1, cl_GMM_c2, cl_test_data(:,cl_idx), classes);
confusionmat(cl_test_labels, p)
disp(['Accuracy TEST: ' num2str(mean(p == cl_test_labels))])
