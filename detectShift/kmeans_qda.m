...
addpath('/home/paolo/cvsa/ic_cvsa_ws/src/analysis_cvsa/utils')
...
for c = 1:ntrial
    ...
    for sample = 1:min_trial_data
        ...
        for idx_band = 1:nbands
            ...
            [sparsity(sample, idx_band, c,:), label_sparsity] = compute_features_icnic(tmp);
        end
    end
end
...
[idx_train, C] = kmeans(train_data_2D_noArtif, K, ...
    'Distance', 'sqeuclidean', ...
    'Replicates', 10, ...  % 'Replicates' è fondamentale per un risultato robusto
    'Display', 'final');

[~, sortIdx] = sort(C(:, 1), 'descend'); % Ordina in base alla feature 2 (OFI)
C = C(sortIdx, :);
...
cluster_labels_train_kmeans = nan(size(sparsity_cf, 1), ntrial_train);
for c = 1:ntrial_train
    cluster_labels_train_kmeans(:,c) = raw_labels((c-1)*size(sparsity_cf, 1) + 1: c * size(sparsity_cf, 1));
end
...
%     ylim([0, 1])
    ...
    subplot(4,1,3)
    plot(squeeze(cluster_labels_train_kmeans(:, c)), 'b')
    legend('kmeans')
    xticks(0:sampleRate:min_trial_data)
    xticklabels(string((minDurFix+minDurCue+1:sampleRate:min_trial_data) / sampleRate));
    xlim([1 size(sparsity_cf, 1)])
    ...
    if c <= ntrial_train
        for sample = 1:size(c_data,1)
            if artifacts_cf(sample,c) == 0
                if cluster_labels_train_kmeans(sample, c) == 1 % IC
...
for idx_ch=1:nchannels-1
    ...
    fisher_IC(idx_ch) = abs(mu1 - mu2)^2 / (sigma1^2 + sigma2^2);
    ...
    fisher_cl(idx_ch) = abs(mu1 - mu2)^2 / (sigma1^2 + sigma2^2);
end
...
figure();
imagesc(tmp')
title('kmeans ic and classical fisher score')
colorbar;
yticks(1:nchannels); yticklabels(channels_label)
...
for c = ntrial_train+1:ntrial
    ...
    subplot(4,3,4)
    if c <= ntrial_train
        c_cluster = squeeze(cluster_labels_train_kmeans(:, c));
    else
