%% CNN with covert visual attention
clear all; % close all
...
[h, w] = size(data);
Xnew = reshape(data, h, w, 1, 1);
out_sdn = classify(decoder.net, Xnew);


function [net] = errp_decoder_sdn(train, train_labels)
    inupt_size = size(train);
    inupt_size(3) = 1;
    numClasses = length(unique(train_labels));  % example number of classes

    Y = categorical(train_labels);

    X_reshaped = reshape(train, inupt_size(1), inupt_size(2), 1, []);

    [XTrain, YTrain, XVal, YVal] = splitData(X_reshaped, Y, 0.8);

    layers = [
        imageInputLayer(inupt_size)
...
function [XTrain, YTrain, XVal, YVal] = splitData(X, Y, trainRatio)
    % Validate inputs
    if numel(Y) ~= size(X, 4)
        error('Number of labels must match number of samples in X.');
    end
